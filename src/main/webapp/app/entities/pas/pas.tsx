...
import { getEntities } from './pas.reducer';
import { IPAS } from 'app/shared/model/pas.model';
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';
import { ITEMS_PER_PAGE } from 'app/shared/util/pagination.constants';
import { overridePaginationStateWithQueryParams } from 'app/shared/util/entity-utils';

export interface IPASProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export const PAS = (props: IPASProps) => {
  ...
  return (
    <div>
      <h2 id="pas-heading" data-cy="PASHeading">
        PAS
      ...
      <div className="table-responsive">
        {pASList && pASList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('tipo_proveedor')}>
                  Tipo Proveedor <FontAwesomeIcon icon="sort" />
                </th>
            ...
            <tbody>
              {pASList.map((pAS, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`${match.url}/${pAS.id}`} color="link" size="sm">
                      {pAS.id}
                    </Button>
                  </td>
                  <td>{pAS.cif}</td>
                  <td>{pAS.nombre}</td>
                  <td>{pAS.codigo_postal}</td>
                  <td>{pAS.email}</td>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${pAS.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${pAS.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">No PAS found</div>
        )}
      </div>
      {props.totalItems ? (
        <div className={pASList && pASList.length > 0 ? '' : 'd-none'}>
...
export default connect(mapStateToProps, mapDispatchToProps)(PAS);
